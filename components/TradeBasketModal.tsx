...
type Result = { ticker: string; ok: boolean; positionId?: number; error?: string };
      ...
      <div className="relative w-full max-w-lg rounded-lg border border-border bg-surface shadow-2xl">
        ...
        {step === "executing" && (
          <div className="p-8 flex flex-col items-center gap-3">
            ...
            <div className="text-[13px] text-fg-muted">Submitting orders to eToro…</div>
          </div>
        )}
            ...
            <table className="w-full text-[12.5px] tabular-nums border border-border rounded-md overflow-hidden">
              <thead className="bg-bg-soft text-fg-subtle">
                <tr>
                  ...
                  <th className="text-left px-3 py-2 font-mono uppercase tracking-[0.14em] text-[10px]">Ticker</th>
                  <th className="text-right px-3 py-2 font-mono uppercase tracking-[0.14em] text-[10px]">Status</th>
                </tr>
              </thead>
              <tbody>
                {results.map((r) => (
                  <tr key={r.ticker} className="border-t border-border">
                    <td className="px-2 py-2">
                      {r.ok ? (
                        <Check className="w-3.5 h-3.5 text-zone-clear" />
                      ) : (
                        <X className="w-3.5 h-3.5 text-zone-storm" />
                      )}
                    </td>
                    <td className="px-3 py-2 text-fg">{r.ticker}</td>
                    <td className="px-3 py-2 text-right text-fg-muted">
                      {r.ok ? `Filled #${r.positionId ?? "—"}` : (r.error ?? "Failed")}
                    </td>
                  </tr>
                ))}
              </tbody>
